...
// output dim3 u16
Func local_laplacian(
    const int pyramid_levels, // default: 8, or between 1 and maxJ
    ImageParam input, // dim3 u16
    Param<int> levels,
    Param<float> alpha,
    Param<float> beta
) {
...
int main(int arc, char** argv) {
    Func input_;
    input_(x, y, c) = cast<uint16_t>(random_int());
    Buffer<uint16_t> input = input_.realize(W, H, 3);

    printf("---- input ----\n");
    for (int c = 0; c < 3; c++) {
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x ++) {
                printf("%d ", input(x, y, c));
            }
        }
    }
    printf("----\n");

    ImageParam input_param(UInt(16), 3);
    ParamMap params;
    params.set(input_param, input);
    Target t = get_jit_target_from_environment();
    Buffer<uint16_t> output = local_laplacian(
        pyramid_levels,
        input_param,
        Param<int>(levels),
        Param<float>(alpha / (levels - 1)),
        Param<float>(beta)
    ).realize(W, H, 3, t, params);
